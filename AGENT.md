...
### 10.5 时间与时区（有跨时区语义的界面必读）

**界面上一律按北京时间（UTC+8）判定与展示，且必须与浏览器本地时区无关。**
供应商的定价/刷新/重置规则几乎都以 UTC 给出，而用户看的是中国时间；用 `getHours()`、
`getDay()`、`toLocaleString()` 直接读本地字段，会让同一份数据在不同机器上算出不同结论。
