...
| **[@dshp/file-change-viewer](plugins/file-change-viewer/README.md)** | 文件修改查看器：接管对话流里的 edit / write 工具卡片并**默认展开**，直接显示变更行、文件路径与增删行数（进行中的调用也能看到正在写入的内容）                                          | [README](plugins/file-change-viewer/README.md) |

## 截图预览

### `@dshp/token-meter` — 额度 + 用量 + 在线（界面名：**Token 总览**）

在**会话中心区**注册一个 tab（`conversation.view`，与原生「对话 / 轨迹」并列），tab 内用**左侧菜单**切换四块：

- **额度查询**：峰谷提醒横幅 + 全部供应商富卡片（配额进度条按 5 小时 / 每周 / 每月分级，标注占比与重置倒计时，余额不足触发红色预警）；不按启用/禁用分组，全部混排在一个随宽度 4/3/2/1 列的自适应网格里。
- **用量统计**：基础数据指标卡 + Token 趋势折线 + 活动热力图 + 模型用量分布（圆环 + 多列模型列表，进度条与印出的百分比同口径）+ 每日在线 + 每日在线排行。
...
| 额度查询                                          | 用量统计                                                | 在线统计                                           |
| ------------------------------------------------- | ------------------------------------------------------- | -------------------------------------------------- |
| ![额度查询](plugins/token-meter/images/quota.png) | ![用量统计](plugins/token-meter/images/token-usage.png) | ![在线统计](plugins/token-meter/images/online.png) |
...
│   ├── file-change-viewer/     # @dshp/file-change-viewer（edit / write 卡片默认展开的文件变更视图）
│   │   ├── src/host/           #   Host TS：仅 name/inject/apply（bundle 入口 + dsh.client 发现锚点）
│   │   ├── src/client/         #   Client TS：diff（纯推导）+ FileChangeRow（DiffBlock 渲染）+ styles
│   │   ├── scripts/            #   check-client.mjs（无头渲染 48 项断言）
│   │   └── lib/                #   单文件构建产物（已提交）
