...
| 写法               | 作用                                                                                                                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `/mcwiki <搜索词>` | 全文搜索，并**附第一条条目的正文详情**（整页纯文本前 6000 字——wiki 的引言段往往只有一两句，整页才有分节正文；详情抓取失败自动降级为纯列表） |
| `/mcwiki`          | 显示用法                                                                                                                                    |

刻意**不做子命令**（`read`/`random` 等）：输入框对子命令没有补全提示，用户发现不了；想看其他条目就再搜一次 `/mcwiki <条目名>`，随机与全文需求由模型工具 `mcwiki_random` / `mcwiki_get_page` 承接。搜索条数沿用设置页配置（`searchMaxResults`）；请求跟随命令的取消信号。
