...
>
> 官方 DiffBlock 进卡后还会自带三样**冗余**，被结构性清掉（选择器全走它自带的 `data-diff` 根 + 子元素位置）：body 首行那条**完整路径**（官方 buildRows 无条件插入，600 字重 + 56px 右留白，读起来像正文里横插的标题）、右上角**复制按钮**（卡头视图切换旁再浮一个）、块底 **`└ +A -B · N 个文件` footer**（与卡头统计同口径重复报数）。卡头已有文件名与统计，这三样只会在上下文与改动之间制造噪声。
>
> 上下文行虽然不是官方组件画的，但**读起来必须像同一块代码**，所以做了三件事对齐：① 排版逐像素镜像官方——同一字体 token（`--dsw-font-markdown-code-block`，实测 11px/19px + code 字族）、行盒与官方 `.line` 同为 22px 最小盒高；② 衔接处零缝隙——有上文时把 DiffBlock 改动区自己的顶部内边距归零（`+ .ctx + .diff > div:first-of-type`），上下文行与 ± 行逐像素相接；③ **横向滚动收敛到一处**——外层 `diffWrap` 是整份 diff（上文 + 改动 + 下文）唯一的滚动容器：官方 `.body` 的滚动被结构性覆盖关掉（`data-diff` 根下第一个 div，注意**不能写 `:first-child`**——body 前面还有个复制按钮 `<button>`），上下文行不自己滚。超长行因此只有**一根滚动条、一条轴**，不会出现「上下文一根条、改动另一根条、各滚各的」。
>
> **高亮视图的行底色铺满整条滚动宽度**：官方 `.line` 是普通块盒，宽度只到 `code` 可视宽度；`white-space:pre` 下超长文字以行内溢出延伸到盒外——直接在 `.line` 上叠底色的话，往右滚，红绿条会在行中间**断开**（文字还在、底色没了）。所以行盒被显式撑宽：`code > :global(.line) { box-sizing:border-box; width:max-content; min-width:100% }`——短行底色照旧铺满整卡，长行底色跟着文字走到行尾。**`border-box` 必须显式写**：行号列是 `.line` 自己的 `padding-inline-start`，默认 content-box 下 `min-width:100%` 只约束内容宽，行号内边距叠加在外，背景会凭空多出约一个行号列宽。`.line` 是官方全局类名（官方自己的 CSS Module 里就是 `:global(.line)`），所以这里必须写 `:global` 形态，构建产物里它是合法的裸类名（`check-client.mjs` 的哈希护卫已为它豁免）。
>
> **行号列从卡片左边框让出一档 14px**：官方把行号画在行盒的最左缘（绝对定位的 `::before` 落在 `inset-inline-start:0`），而上面为了让底色铺满整卡把 `pre` 的横向内边距归零了——官方默认那 16px 一拿掉，行号列就紧贴卡片边框。行号列宽是 `max(2, 位数)ch`（官方按代码行数内联写在块根上），位数一多（3–4 位）数字会把整列填满，等于压在边框上。所以行盒补一条 `padding-inline-start: calc(var(--dsl-code-block-line-number-width) + 26px)`（26px = 新的 14px 档 + 原有的 12px 行号/正文间隙），行号本身用 `.line::before { inset-inline-start: 14px }` 同步右移。**14px 不是随手取的数**：它正是本插件另一套版式那一档（`.ctx` 的 `padding:0 14px`、官方 `DiffBlock` 的 `.body{padding:12px 14px}`），于是高亮与 ± 两个视图的行号列、正文列落在同一个 x 上（无头 Chrome 像素实测：两视图的行号 ink 都自卡片左缘 +15px 起、正文 ink 同一 x）。设置页样张的 `.pvCode` 复刻同两条规则，样张与真用起来一致。
>
> **± 差异视图：上下文行带真实行号，改动行的行号列放 `+` / `-`**。官方 `DiffBlock` 的行没有编号机制，上下文行的行号由本插件用 CSS 计数器（`dshp-fcv-num`）补上——上文从「上文首行的真实行号」起数，下文的起点 = 上文首行 + 上文行数 + 语义新增行数（删除行不占号，与高亮视图同口径）；起点由 JSX 内联成 `.ctx` 容器上的 `counter-reset`，定位不到（`locate` 没回结果）时不带号、绝不编数。行号列的宽窄（3ch + 12px 间隙）、颜色与淡化与高亮视图的官方行号一致。
>
> **改动行不编号，而是把 `+` / `-` 画在同一列里**：`diffSignRules` 生成的动态 CSS 给 del / add 行让出与上下文行号**同一列**（`padding-inline-start: calc(3ch + 12px)`），并把官方行内的 `- `/`+ ` 前缀压掉（`::before{content:none}`），改在自己的 `::after` 里画符号（del 红 / add 绿）——于是「上下文行号 / 改动符号 / 正文」三列严格对齐（无头 Chrome 实测：三行正文左缘都是同一个 x）。这里还有一条**必需**的配套规则：改动行自己得是那个 `::after` 的包含块（`.diff[data-diff] > div > div { position:relative }`）——官方行是 static，不定位的话 `inset-inline-start:0` 会向上找到官方块根（整张卡）的左缘，`+` / `-` 就比上下文行号左出一档、还紧贴卡片边框（行号列与符号列错开 14px）。不这样做的话，官方的行内前缀会把改动行的正文多推两个字符，上下文行与改动行的正文就错开了。区间用 `div:nth-of-type` 而非 `nth-child`：官方块超长折叠时行间会插一个 `<button>`（展开其余行），`:nth-of-type` 按元素类型计数、button 不占 div 的位，索引与行序严格一致；已隐藏的 path 行仍占 div 索引（display 不影响选择器），所以区间统一 +1 偏移。设置页的样张不传 hunk 类名，所以样张里的改动行保持官方原样（行内前缀、不与上下文对齐）。
