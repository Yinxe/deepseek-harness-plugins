...
>
> **改动行（官方块内）同样带双侧行号**：官方块内的 del / add 行由 `diffNumRules` 生成的动态 CSS 补号——沿 hunk 的 LCS 结果走一遍（ctx 行双侧行号都推进、del 只推进旧侧、add 只推进新侧）得到「首个 del 的旧文件行号」与「首个 add 的新文件行号」，计数器 `dshp-fcv-old` / `dshp-fcv-new` 从这里起数，行号画在 `::after`（绝对定位到行盒左缘，与上下文行号同列；del 行的号着红、add 行的号着绿，与 ± 前缀符号同色，一眼可辨是哪侧的号）。区间用 `div:nth-of-type` 而非 `nth-child`：官方块超长折叠时行间会插一个 `<button>`（展开其余行），`:nth-of-type` 按元素类型计数、button 不占 div 的位，索引与行序严格一致；已隐藏的 path 行仍占 div 索引（display 不影响选择器），所以区间统一 +1 偏移。行号不可信（定位不到）时改动行同样不带号。
