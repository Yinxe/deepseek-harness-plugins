...
补丁格式是 **Codex / opencode `apply_patch` 的 `*** Begin Patch` 信封**，不是 `patch(1)` 的 unified diff：
...
| 性质     | 说明                                                                                                                                                                                                                      |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 格式     | `*** Begin Patch` / `*** End Patch` 信封，段落是 `*** Add File:` 与 `*** Update File:`；一次可带**多个文件、每个文件多个 `@@` 片段**                                                                                      |
| 定位     | **不写行号**：片段靠自己的上下文内容定位，**五级回退**（精确 → 去尾空格 → 两端 trim → Unicode 标点归一 → 忽略一切空白但**要求全文件唯一**）；`@@ 锚点` 用于消歧——**锚点写成本段第一行也认**；`*** End of File` 钉在文件尾 |
| 追加     | 只有 `+` 行的片段（没有上下文、没有删除）**追加到文件末尾**——这是上游行为，照抄                                                                                                                                           |
| 新建文件 | `*** Add File: <路径>` + 整段 `+` 行，父目录自动创建                                                                                                                                                                      |
| 原子性   | **全有或全无**：先把每个文件读进来、在内存里全部应用成功才落盘；任一片段对不上就整体失败，绝不写一半                                                                                                                      |
| 后端     | 走 `ctx.fs`：同样吃沙箱模式、`fs/write-intent` 观察策略、`fs/observed` 记账；不绕过 seam                                                                                                                                  |
| 行尾/BOM | CRLF 文件归一后应用、写回时恢复整份 CRLF；带 UTF-8 BOM 的文件改完 BOM 还在                                                                                                                                                |
| 范围     | **只做「新建 / 修改」**：`*** Delete File:` 与 `*** Move to:` 解析层认识、执行层明确拒绝（见下）                                                                                                                          |
| 渲染     | 结果元数据带 `diffs`，于是这张调用也用它自己的差异卡片显示（按片段各一张卡、统计只算真正变化的行、行号是**落盘时算出的真实行号**）                                                                                        |

### 一次调用就成功的几条设计

模型第一次调用 `patch` 失败、第二次才改对，是最常见的浪费。真实会话里抓到的两类根因，以及对应的处理：

| 现象                                                                                     | 根因                                                                                                                                               | 现在怎么处理                                                                                                                                                      |
| ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 写了 `@@ Line 1: Hello DSH`，下一行又是 `-Line 1: Hello DSH`，报「找不到这一段的上下文」 | 模型把 `@@` 当**小节标题**用（像 unified diff 的 hunk 头），而它是**锚点**；旧实现匹配到锚点后从**下一行**开始找，于是要改的那一行被锚点本身"吃掉" | 锚点命中后从**锚点自身**开始找：锚点是上方一行 → 继续往下找；锚点就是改动行 → 当场命中。工具描述也把「锚点是**文件里真实存在的一行**、不是标题」写明白了          |
| 凭记忆写的那一行与文件只差几个空格（`.dashing{` vs `.dashing {`），报「找不到」          | 前四级回退吃缩进与行尾空白，**吃不了「有没有空格 / 空格在哪」**的差异                                                                              | 加第五级：忽略一切空白比较，但配**唯一性安全阀**——全文件只有一处能对上才采用，两处以上就不猜（宁可失败，也不改错地方）                                            |
| 一个片段里堆了多处删除，但它们不相邻                                                     | 片段语义要求旧内容在文件里**连续**；模型把它当成「一次批量」                                                                                       | 报错时点破这一点，并教它「每处单独写一个 `@@` 片段 / 把中间那些没改的行也写成上下文行」（判定是精确的：按顺序都能找到、但彼此不连续）                             |
| 只发了 `*** Begin Patch`，里面是空的                                                     | 生成被截断 / 模型先写信封再想内容                                                                                                                  | 报错分开说：**空补丁**（教它段落怎么写）、**缺 `*** End Patch`**（提示可能被截断）、**整段写成 unified diff**（给出可照抄的正确格式，并提醒小改用 `edit` 更省事） |
| 重试时又发了一遍同样的改动                                                               | 上一次其实已经改好了                                                                                                                               | 检测「**新内容已经在文件里、旧内容不在**」，提示「这段改动看起来已经应用过，确认后把这一段从补丁里去掉」（只是提示，不动磁盘）                                    |

**报错不再只说「找不到」**：`applyChunksToText` 失败时会在文件里找与模型写的那一行**最像的真实行**，把行号、原文与「差在哪」一起回给模型：

```text
patch 无法应用：在 /path/to/s.css 里找不到这一段的上下文：
  .dashing{animation:dashFlash .18s ease-out;}
文件第 64 行最像（相似度 94%），实际是：
    .dashing { animation: dashFlash .18s ease-out; }
（只有空格 / 制表符的差别（有没有、在哪里））
把这一行**原样**（连同缩进）抄进 - 行再试一次；改动上下各留一行上下文会更稳。
文件没有被修改。
```

三段信息各有用途：原文回显让它能对照；文件里的真实行让它**照抄就能过**；差异原因（缩进 / 行内空白 / 标点 / 第几个字符起不同）让它知道**下次别再凭记忆写**。相似度不够高时改成明确提示「这段文本在文件里不存在，先 read 再写」。诊断只在失败路径上跑，并且有行数上限（2 万行以上不扫）。

**为什么不做删除与改名**：这两件事本来就有官方出口——`bash` 的 `rm` / `mv`（git 跟踪的用 `git rm` / `git mv`），而且与 `patch` 吃同一份沙箱策略，越界一样会被拒。反过来，把它们塞进 `patch` 的收益只是省一次调用，代价却很实：

- **删除不可逆**，而 `patch` 的「全有或全无」只覆盖**验证阶段**——落盘仍是逐个执行的，第 3 个文件失败时前两个已经改了、删掉的也回不来；
- **改名**同样会不可逆地动到另一个路径；
- 跨平台另有一套命令转义要维护（POSIX `rm` 与 Windows 的 `pwsh` 语法不同），等于把一段在 CI 上跑不到的代码交付出去。

所以模型真写出这两段时，`patch` 会在**读任何文件、问任何审批之前**整体拒绝，并给出替代做法：
...
客户端仍然把「它本来打算做的改名」画出来（卡头 `旧 → 新`）并打 rejected 角标——用户要看到的是模型想干什么。

> 什么时候用哪个：一处小改动 → `edit`；新建 / 整份重写 → `write`；**多处零散或跨文件 → `patch`**；删除 / 改名 / 构建 / 测试 / git → `bash`。

## 两种展示方式（设置 → File Change View）

| 方式             | 组件                            | 形态                                                                                                                                                                  |
| ---------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **高亮**（默认） | 官方 `CodeBlock`                | 单个代码块里放完整统一 diff（LCS 结果）：未变行只出现一次，删除行红底、新增行绿底，删行不占行号；**代码原样**（不加 `-` / `+` 标记）                                  |
| **± 差异**       | 官方 `DiffBlock` + 中性上下文行 | 逐行 `-`/`+` 红绿文字（紧凑），超长中部折叠；自带复制按钮与 `└ +A -B` 页脚。喂给它的是**语义变更**（只含真正变化的行），所以它的 `+A -B` 与高亮视图、行头统计永远一致 |

设置页里这两种方式是**两张并排的单选卡**，每张卡下面用**同一份渲染代码**（`diffView.ts`，与工具行走的就是这一套）画一段真实样张（`src/greet.ts` 的一次 `+2 -1` 小改），选中即生效——挑的时候看到什么，会话里就是什么。卡片整块可点（样式里给样张加了 `pointer-events:none`，官方组件自带的复制按钮在样张里不会抢走点击）。

两个视图都会在改动两侧**多显示几行没受影响的上下文**（`contextLines`，默认 3 行）。这几行**取自文件当前内容**——Host 的 `/locate` 路由拿文件定位这段 hunk 时，顺手把它两侧各最多 8 行原文回给客户端（响应里只有行号与这十几行，**整份文件不回传**），客户端按偏好截。

> **为什么 ± 差异视图的上下文是「另外画的一行行中性文字」**：官方 `DiffBlock` 的行 kind 只有 `path | del | add | gap`（别的 kind 直接 throw），**画不了中性的上下文行**——上下文一旦塞进它的 `diffs`，就会变成绿色的 `+` 或红色的 `-`，把没改的行说成改了。所以本插件把上下文单独渲染成中性行贴在它上下：`上文 → DiffBlock（红绿改动） → 下文`，拼起来正好是一份标准的 unified diff。

### 三层优先级：单块点击 > 会话级覆盖 > 全局偏好
...
会话标题栏右侧（官方「在应用中打开」、会话日志下载的同一排）多出一组胶囊按钮：

```
[ ⌄ 收起改动 ] | [ </> 高亮 ]  ↺      ← 当前会话已被临时改动（↺ = 恢复跟随偏好）
[ › 展开改动 ] | [ ⑂ ± 差异 ]         ← 跟随设置页里的全局默认
```
...
「全局偏好里的默认展开」只影响**之后新渲染**的编辑 / 写入操作（已经在会话里的行保持原样，不会被开关抽走或撑开）；想对**当前这个会话**立刻生效，用页头那两个开关。行内不再放全局切换按钮——逐行记忆只会让界面自相矛盾。
...
| 场景                           | 表现                                                                                                                                                                                                                                         |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 行的默认状态                   | 渲染那一刻的**会话级覆盖 → 全局偏好**：默认**折叠**（与思考 / 读取行一致）；打开「编辑 / 写入默认展开」后新渲染的行直接展开，或点会话页头的「展开改动」立刻摊开当前会话的全部改动；点行任意处或 Enter / Space 可随时单独切换（临时，不写回） |
| `write` 新建文件               | 整份内容绿底 + 高亮，行头统计显示 `+N -0`                                                                                                                                                                                                    |
| `write` 覆盖已有文件           | 用结果元数据的真实 hunk，再按偏好补上文件里前后各 N 行上下文                                                                                                                                                                                 |
| `edit` 改文件                  | 一个代码块内交错：红底旧行（不占行号）、绿底新行、上下文行不染色，全部带语法高亮；`replace_all` 在行头提示；行号是文件里的真实行号（上下文行一并占号）                                                                                       |
| 上下文行（两个视图）           | 改动的上下各 N 行（`contextLines`，默认 3）取自**文件当前内容**；高亮视图并进同一个代码块（行号相应前移），± 差异视图渲染成中性行贴在官方 `DiffBlock` 上下                                                                                   |
...
| 每个文件块                     | 自己一条原生折叠行（路径 + 该块统计），默认开合跟随会话级覆盖与用户偏好（单独点过的块由用户说了算），可临时点开 / 收起                                                                                                                       |
...
| ------------ | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 展开态       | 默认收起                            | 默认收起；设置里可把**编辑 / 写入**改成默认展开（只影响之后新渲染的行），会话页头的「展开 / 收起改动」可立刻摊开**当前会话**的全部改动                                                     |
...
| 偏好           | 控件           | 默认        | 说明                                                                                                                                                                                                                                                                                                                                                                     |
| -------------- | -------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `view`         | 两张预览单选卡 | `highlight` | `highlight` = 单代码块统一 diff（整行红绿 + 行号 + shiki 高亮）；`diff` = 逐行 ± 视图。卡里直接画出两种效果                                                                                                                                                                                                                                                              |
| `contextLines` | 下拉选择框     | `3`         | **改动两侧各多显示几行没受影响的上下文**（`0` / `3` / `5` / `8`）。这几行**取自文件当前内容**（Host 的 `/locate` 路由定位后一并回传），不是模型在 `old_string` / 补丁片段里带的那几行——模型只圈 1 行上下文时也能看清改动落在哪里；读不到文件、或这段改动之后又被改过时不补上下文（不编内容）                                                                             |
| `sectionsOpen` | 开关           | `false`     | **新渲染的「编辑 / 写入」操作是否默认展开**（开 = 直接看到改动，行内文件块也默认展开；关 = 与思考 / 读取行一致，点一下才展开）。键名是历史遗留，含义已收敛为「这一行要不要默认展开」；只作用于**新渲染**的行，且只作用于本插件接管的编辑 / 写入行，`bash` / `read` 等其它行不受影响。会话页头的「展开 / 收起改动」是**另一个入口**（只压当前会话、不落盘），两者互不影响 |
...
dshp-file-change-viewer:
  ...
  patchTool: false # 是否注册 patch 工具（测试版，默认关）
```

> **动态开关改的是「注册」而不是「配置」**：`patchTool` 一开一关，Host 会真的注册 / 反注册这个工具。
> 判定「现在该不该注册」时问的是 **settings 服务的权威值**（`settings.get(NS)`，schema 默认 + base 层 + 用户层的合并结果），
> 而不是 `setSource` 交付的那个 thunk——真实 settings 在 attach / detach 时也会用「只有 base 层」的源回调一次，
> 只看 thunk 会在那一刻把工具误反注册（真实会话里表现为模型收到 `unknown tool "patch"`）。
> `scripts/check-host.mjs` 有断言守着这条，并且反向验证过：把逻辑改回旧样子，该断言就会挂。
...
| 常量                            | 位置                          | 值 / 来源                                     | 含义                                                                                     |
| ------------------------------- | ----------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `DEFAULT_CONFIG`                | `src/host/config.ts`          | 与 schema 默认同值                            | 出厂默认（`highlight` / `contextLines: 3` / `sectionsOpen: false` / `patchTool: false`） |
...
├── src/host/patch-tool.ts       # patch 工具：*** Begin Patch → ctx.fs（全有或全无 + 差异元数据 + 删/改名拒绝）
├── src/host/types.ts            # Host 半类型（PluginConfig / PluginConfigPatch / AnyCtx）
├── src/shared/apply-patch.ts    # *** Begin Patch 解析 / 应用 / 行尾（纯函数，Host 与 Client 两份产物共用）
├── src/client/index.ts          # Client 入口：__ModuleLoader__ + 样式注入 + 槽位注册（设置节 + 页头开关 + 4 个工具 key）
├── src/client/FileChangeRow.ts  # 工具行：原生 DisclosureRow + 每文件块折叠 + 三层优先级（块 > 会话 > 偏好）
├── src/client/FileChangeViewerSection.ts # 设置 → 「File Change View」一节（单选卡 + 下拉 + 开关）
├── src/client/viewCards.ts      # 展示方式两张单选卡 + 卡里直接画出来的真实样张
├── src/client/SessionControls.ts # 会话页头：「展开 / 收起改动」+「差异视图」两个快捷开关
├── src/client/session.ts        # 会话级覆盖（内存、不落盘、换会话即失效）
├── src/client/diffView.ts       # 两种差异视图的渲染（工具行与设置页样张共用同一份代码）
├── src/client/prefs.ts          # 偏好：客户端 store（订阅 + 乐观写 + 失败回滚）↔ /ext 路由
├── src/client/api.ts            # /ext 路由封装（fetchState / saveConfig）
├── src/client/locate.ts         # 真实行号 + 上下文：批量问 Host + 缓存 + 订阅（定位不到就回 1 起、不补上下文）
...
- **卡片上改动前后那几行灰字是什么**：是**上下文**——从文件当前内容里取出的、没受影响的上下各 N 行（默认 3，设置 → File Change View → 「上下文行数」可改 0 / 3 / 5 / 8）。模型给 `edit` 的 `old_string` 常常只圈 1 行（它只要保证唯一），补丁片段也只带自己那点上下文，所以这几行是**本插件补的**，不是模型写的；它们不参与 `+A -B` 统计，也不是变更内容。文件读不到（比如刚被删了、路径在窗口之外）或这段改动之后又被改过时就不补（宁可没有，也不编内容）。
- **行号是文件里的真实行号吗**：是。官方代码块的行号由 CSS 计数器给出、默认从 1 数起（`counter-reset:source-line` 挂在 `code` 上），本插件在这一块自己的类名下标一次（特异性更高）：`counter-reset:source-line <起始行号 - 1>`，于是首行编号就是它在文件里的真实行号；删除行照旧不占号、不推进计数器。起始行号有三个来源，按可靠度取：① 本插件 `patch` 工具**落盘时算出的片段落点**（确切：片段自己的位置 + 前面片段造成的行数漂移）；② edit / write 的元数据里**没有**偏移（官方 `computeHunkDiffs` 只给 `{path, oldText, newText}`），于是向 Host 的 `/ext/dshp-file-change-viewer/locate` 问一次——Host 拿文件当前内容定位这段 hunk 的首行，**响应里只有整数、不回文件内容**，结果按 `path + newText` 缓存；③ 定位不到（文件之后又被改过）就老老实实从 1 开始，不编一个错的数字。行头的路径链接也会带上这个行号，点开直接落在改动处。
...
>   5. 新增**全局偏好**（展示方式下拉 + 编辑 / 写入是否默认展开），注册成设置里**独立的一节**（`settings.section`），读写走自有 `/ext` 路由 → Host `settings.update`，落 `settings.yaml` 的 NS 分节；不再有行内切换按钮，也不占用「插件」tab。默认展开只影响**新渲染**的行。
