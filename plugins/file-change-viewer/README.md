...
>
> **改动行不编号，而是把 `+` / `-` 画在同一列里**：`diffSignRules` 生成的动态 CSS 给 del / add 行让出与上下文行号**同一列**（`padding-inline-start: calc(3ch + 12px)`），并把官方行内的 `- `/`+ ` 前缀压掉（`::before{content:none}`），改在自己的 `::after` 里画符号（del 红 / add 绿）——于是「上下文行号 / 改动符号 / 正文」三列严格对齐（无头 Chrome 实测：三行正文左缘都是同一个 x）。不这样做的话，官方的行内前缀会把改动行的正文多推两个字符，上下文行与改动行的正文就错开了。区间用 `div:nth-of-type` 而非 `nth-child`：官方块超长折叠时行间会插一个 `<button>`（展开其余行），`:nth-of-type` 按元素类型计数、button 不占 div 的位，索引与行序严格一致；已隐藏的 path 行仍占 div 索引（display 不影响选择器），所以区间统一 +1 偏移。设置页的样张不传 hunk 类名，所以样张里的改动行保持官方原样（行内前缀、不与上下文对齐）。
