...
>
> **± 差异视图的上下文行也带真实行号**：官方 `DiffBlock` 的行没有编号机制，行号由本插件用 CSS 计数器（`dshp-fcv-num`）补上——上文从「上文首行的真实行号」起数，下文的起点 = 上文首行 + 上文行数 + 语义新增行数（删除行不占号，与高亮视图同口径）；起点由 JSX 内联成 `.ctx` 容器上的 `counter-reset`，定位不到（`locate` 没回结果）时不带号、绝不编数。行号列的宽窄（3ch + 12px 间隙）、颜色与淡化与高亮视图的官方行号一致。
>
> **改动行（官方块内）里只有新增行带行号**：官方块内的行由 `diffNumRules` 生成的动态 CSS 补号——沿 hunk 的 LCS 结果走一遍（ctx 行推进新侧行号、**del 行不推进**、add 行推进）得到「首个 add 的新文件行号」，计数器 `dshp-fcv-new` 从这里起数，号画在 add 行的 `::after`（绝对定位到行盒左缘的 4ch 行号列里，右对齐、淡化、不可选中）。**删除行不编号**：它拿的是旧文件的行号，与官方行首的 `- ` 前缀没有对应关系，画出来只会让人以为那是新文件的行号；但 del 行**照样让出行号列**（`padding-inline-start: calc(4ch + 6px)`），否则 add 行的正文会被行号推右、与 del 行的正文错开，红绿两段看起来就不像一个 diff 了。官方的前缀符号是行内 `::before`，让位之后每行结构是 `[行号] [+/-] 正文`，符号永远落在行号**之后**，不会互相压住。区间用 `div:nth-of-type` 而非 `nth-child`：官方块超长折叠时行间会插一个 `<button>`（展开其余行），`:nth-of-type` 按元素类型计数、button 不占 div 的位，索引与行序严格一致；已隐藏的 path 行仍占 div 索引（display 不影响选择器），所以区间统一 +1 偏移。行号不可信（定位不到）时改动行同样不带号。
