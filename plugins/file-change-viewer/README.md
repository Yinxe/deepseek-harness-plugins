...
> **它是测试版能力，默认不注册。** 到 **设置 → 文件修改卡片 → 工具** 打开「启用 patch 工具（测试版）」，
...
四项（展示方式、上下文行数、编辑 / 写入是否默认展开、patch 工具开关）只有一个入口：**设置 → 左侧导航「文件修改卡片」**一节（前三项是**全局显示默认值**，最后一项是**工具注册开关**）。改动**立即经 `/ext/dshp-file-change-viewer/config` 写进 `settings.yaml` 的 `dshp-file-change-viewer` 分节**（Host 调 `settings.update`），保存成功 / 失败都在设置节里如实反馈。
...
- **偏好设置在哪**：**设置 → 左侧导航「文件修改卡片」**（本插件自己的一节，分「显示」与「工具」两组：展示方式用下拉选择框，编辑 / 写入默认展开、启用 patch 工具用开关；改完立即写入 `settings.yaml` 的 `dshp-file-change-viewer` 分节）。
- **模型看不到 `patch` 工具**：它是测试版，**默认关**。到 **设置 → 文件修改卡片 → 工具**打开「启用 patch 工具（测试版）」即可；也可以在 composition 的 `config:` 里写 `patchTool: true` 让整个部署默认打开。改完**不需要重启** `dsh web`（Host 在 settings 的 `onChange` 里重新判定并即时注册 / 反注册）。反过来，如果开关是开着的、模型却还是说没有这个工具，先确认 `dsh web` 起得来（`lib/host.js` 有没有 `patchTool` 这个字段：`curl -s http://127.0.0.1:3080/ext/dshp-file-change-viewer/state`）。
...
- **卡片上改动前后那几行灰字是什么**：是**上下文**——从文件当前内容里取出的、没受影响的上下各 N 行（默认 3，设置 → 文件修改卡片 → 「上下文行数」可改 0 / 3 / 5 / 8）。模型给 `edit` 的 `old_string` 常常只圈 1 行（它只要保证唯一），补丁片段也只带自己那点上下文，所以这几行是**本插件补的**，不是模型写的；它们不参与 `+A -B` 统计，也不是变更内容。文件读不到（比如刚被删了、路径在窗口之外）或这段改动之后又被改过时就不补（宁可没有，也不编内容）。
