...
import type { AnyPrimitives, AnyReact, CheckResponse, VisionConfig, VisionModel, VisionRoute } from './types.js';
...
export function createVisionSection(React: AnyReact, P: AnyPrimitives): () => any {
  ...
  function VisionSection(): any {
    ...
    if (config?.primary) {
      const hasP = modelOptions.some((m) => m.provider === (config.primary as VisionRoute).provider && m.model === (config.primary as VisionRoute).model);
      if (!hasP) {
    ...
    const primarySelect = React.createElement(Select, {
      ...
      options: modelOptions.map((m: VisionModel) => ({ id: m.provider + '///' + m.model, label: routeLabel(m) })),
    ...
    const fallbackSelect = React.createElement(Select, {
      ...
      options: [{ id: '__none', label: '不使用 fallback' }].concat(models.map((m: VisionModel) => ({ id: m.provider + '///' + m.model, label: routeLabel(m) }))),
