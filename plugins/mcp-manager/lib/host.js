...
// src/host/config.ts
var NS = settingsNamespace("dshp-mcp-manager");
...
function isPlainJson(v, depth = 0) {
  ...
  if (isRecord(v)) {
    ...
    return keys.length <= 64 && keys.every((k) => k.length <= LIMITS.envKey) && keys.every((k) => isPlainJson(v[k], depth + 1));
  }
  return false;
}
...
function sanitizePatchFilePath(v) {
...
function parseEnvValueView(v, field) {
  if (typeof v === "string") {
    return v.startsWith("js: ") || v === "js:" ? { js: true, source: v === "js:" ? "" : v.slice(4).slice(0, LIMITS.envValue) } : { js: false, source: v.slice(0, LIMITS.envValue) };
  }
...
function sanitizeServerConfig(raw) {
  ...
  const jsMarks = /* @__PURE__ */ new Set();
  ...
  if (Object.hasOwn(raw, "toolCallTimeoutMs") && raw["toolCallTimeoutMs"] !== null) {
    ...
    toolCallTimeoutMs = Math.min(36e5, Math.max(1e3, Math.floor(n)));
  }
  let failOnStartupError = null;
...
function locatePatchFile(selfModuleUrl, configured) {
...
function isProfileRoot(dir) {
  ...
  try {
    ...
    const dsh = raw && typeof raw === "object" ? raw["dsh"] : void 0;
    return dsh !== null && typeof dsh === "object" && Object.hasOwn(dsh, "profile");
  } catch {
    return false;
  }
}
function scanProfilesForSelf() {
  try {
    ...
    for (const name2 of names) {
      const pkgPath = join(profilesDir, name2, "package.json");
      if (!existsSync(pkgPath)) continue;
      try {
        const raw = JSON.parse(readFileSync(pkgPath, "utf8"));
        const dsh = raw && typeof raw === "object" ? raw["dsh"] : void 0;
        const profile = dsh && typeof dsh === "object" ? dsh["profile"] : void 0;
        const bundles = profile && typeof profile === "object" ? profile["bundles"] : void 0;
        if (Array.isArray(bundles) && bundles.includes("@dshp/mcp-manager")) {
          return join(profilesDir, name2);
        }
      } catch {
      }
    }
...
var JS_TAG_DEFS = [
  ...
  tag,
  resolve: (value) => typeof value === "string" ? value : value ?? ""
}));
var PARSE_OPTIONS = { customTags: JS_TAG_DEFS };
var JS_TAG_URIS = /* @__PURE__ */ new Set([
  "tag:yaml.org,2002:js",
  "tag:yaml.org,2002:js/undefined",
  "tag:yaml.org,2002:js/regexp"
]);
function isJsTagged(node) {
  const tag = node?.tag;
  if (typeof tag !== "string") return false;
  return JS_TAG_URIS.has(tag) || tag.startsWith("tag:yaml.org,2002:js/");
}
function parsePatchDoc(text) {
  return (0, import_yaml.parseDocument)(text, PARSE_OPTIONS);
}
function asMapItem(item) {
  if ((0, import_yaml.isMap)(item)) return item;
  return null;
}
...
function pairOf(map, key) {
  return map?.items.find((p) => scalarString(p.key) === key);
}
function findEntries(doc) {
  const out = [];
  const walkSeq = (seq, parentSeq, parentIndex) => {
      ...
      if (m) {
        ...
        if (namePair && scalarString(namePair.value) === MCP_CLIENT_NAME && idPair) {
          ...
          out.push({
            ...
            parentIndex,
            entryMap: m,
            configNode: (0, import_yaml.isMap)(cfgPair?.value) ? cfgPair.value : null,
            id: scalarString(idPair.value).slice(0, LIMITS.id),
            line: lineOfRange(m)
          });
        }
      }
      if ((0, import_yaml.isSeq)(item)) walkSeq(item, seq, index);
      else if ((0, import_yaml.isMap)(item)) walkMap(item, seq, index);
    });
  };
  const walkMap = (map, parentSeq, parentIndex) => {
    for (const p of map.items) {
      if ((0, import_yaml.isSeq)(p.value)) walkSeq(p.value, parentSeq, parentIndex);
      else if ((0, import_yaml.isMap)(p.value)) walkMap(p.value, parentSeq, parentIndex);
    }
  };
  if ((0, import_yaml.isSeq)(doc.contents)) walkSeq(doc.contents, null, -1);
  else if ((0, import_yaml.isMap)(doc.contents)) walkMap(doc.contents, null, -1);
  return out;
}
function lineOfRange(node) {
  const r = node.range;
...
function nodeToJson(node, depth = 0) {
  if (depth > LIMITS.jsonDepth) return void 0;
  if ((0, import_yaml.isScalar)(node)) {
    const v = node.value;
...
function scalarToEnvView(node) {
  if (!(0, import_yaml.isScalar)(node)) return null;
  const raw = node.value;
  const source = raw === null || raw === void 0 ? "" : String(raw);
  return { js: isJsTagged(node), source: source.slice(0, LIMITS.envValue) };
}
function stringMapView(node) {
  if (node === null || node === void 0) return {};
  if (!(0, import_yaml.isMap)(node)) return null;
  const out = {};
  for (const p of node.items) {
...
function firstIssuePush(issues, text) {
  if (issues.length < 8) issues.push(text.slice(0, LIMITS.issue));
}
function entryToView(entry, text) {
  const issues = [];
  ...
  if (disabledPair) {
    ...
    const v = disabledPair.value?.value;
  ...
  if (transport === "stdio") {
    ...
    if (argsNode === null || argsNode === void 0) {
      args = [];
    } else if ((0, import_yaml.isSeq)(argsNode)) {
      args = argsNode.items.map((a) => scalarString(a).slice(0, LIMITS.arg)).slice(0, LIMITS.argsCount);
    } else {
  ...
  if (timeoutNode !== null && timeoutNode !== void 0) {
    const n = Number(timeoutNode.value);
  ...
  if (failNode !== null && failNode !== void 0) {
    const v = failNode.value;
    failOnStartupError = v === true;
  }
  let reconnect = null;
  const reconnectNode = pairOf(entry.configNode, "reconnect")?.value;
  if (reconnectNode !== null && reconnectNode !== void 0) {
    const json2 = nodeToJson(reconnectNode);
    if (json2 === void 0 || typeof json2 !== "object" || Array.isArray(json2)) {
  ...
  for (const p of entry.configNode?.items ?? []) {
    ...
    const json2 = nodeToJson(p.value);
    if (json2 === void 0) {
      firstIssuePush(
        issues,
    ...
    extras[key.slice(0, LIMITS.envKey)] = json2;
  }
  return {
    id: entry.id,
    dup: false,
...
function tagJsNodes(node, prefix, jsMarks) {
  if ((0, import_yaml.isMap)(node)) {
    for (const p of node.items) {
    ...
    for (const item of node.items) tagJsNodes(item, prefix + ".*", jsMarks);
  } else if ((0, import_yaml.isScalar)(node)) {
    if (prefix && jsMarks.has(prefix)) node.tag = "!!js";
  }
}
function buildConfigNode(doc, config, jsMarks) {
  const plain = {};
...
function keyString(pair) {
  const k = pair.key;
  if ((0, import_yaml.isScalar)(k)) {
    const v = k.value;
    return v === null || v === void 0 ? null : String(v);
  }
  return null;
}
function applyServerConfig(doc, entry, config, jsMarks, providedKeys) {
...
function setEntryDisabled(doc, entry, disabled) {
...
function removeEntry(entry) {
  ...
  const wrapper = parent && entry.parentIndex >= 0 ? parent.items[entry.parentIndex] : void 0;
  ...
  if (entry.seq.items.length === 0 && parent && wrapper) {
    ...
    const comment = wrapper.commentBefore;
    if (comment) {
      const next = parent.items[entry.parentIndex];
      if (next) next.commentBefore = comment + (next.commentBefore ? "\n" + next.commentBefore : "");
    }
  }
}
function insertServer(doc, id, config, jsMarks) {
  ...
  const entryNode = doc.createNode({ id, name: MCP_CLIENT_NAME });
...
async function probeStreamableHttp(input) {
  ...
  const raw = await new Promise((resolve3, reject) => {
    const req = (target.protocol === "https:" ? request : request$1)(
      target,
    ...
    return { status: 0, text: "", contentType: "", error: String(e?.message ?? e) };
  });
  ...
  if (payload === null) {
    return {
      ...base,
      latencyMs,
      detail: ("HTTP " + raw.status + "\uFF0C\u4F46\u54CD\u5E94\u4E0D\u662F\u53EF\u89E3\u6790\u7684 JSON-RPC\uFF08content-type: " + raw.contentType.slice(0, 80) + "\uFF09").slice(0, 300)
    };
  }
  const result = isRecord2(payload["result"]) ? payload["result"] : null;
  const rpcError = isRecord2(payload["error"]) ? payload["error"] : null;
  const serverInfo = result && isRecord2(result["serverInfo"]) ? result["serverInfo"] : null;
  const protocolVersion = result && typeof result["protocolVersion"] === "string" ? result["protocolVersion"] : "";
  const state = "ok";
  let detail;
  if (result) {
    detail = "\u63E1\u624B\u6210\u529F";
  ...
  return {
    state,
    detail,
    serverInfoName: serverInfo && typeof serverInfo["name"] === "string" ? serverInfo["name"].slice(0, 120) : "",
    serverInfoVersion: serverInfo && typeof serverInfo["version"] === "string" ? serverInfo["version"].slice(0, 60) : "",
    protocolVersion: protocolVersion.slice(0, 40),
    latencyMs,
    at: Date.now()
  };
}
function parseProbeResponse(text, contentType) {
  const trimmed = text.trim();
  if (!trimmed) return null;
  try {
    const v = JSON.parse(trimmed);
    if (isRecord2(v)) return v;
  } catch {
  }
  if (contentType.includes("text/event-stream")) {
    ...
    for (let i = frames.length - 1; i >= 0; i -= 1) {
      ...
      try {
        const v = JSON.parse(frame.slice(5).trim());
        if (isRecord2(v)) return v;
      } catch {
      }
    }
...
function apply(ctx, rawConfig) {
  ...
  async function snapshotState() {
    ...
    if (text !== null) {
      try {
        ...
        if (doc.errors.length > 0) {
          fileIssue = "patch \u6587\u4EF6\u89E3\u6790\u6709\u95EE\u9898\uFF08" + String(doc.errors[0]).slice(0, 200) + "\uFF09\uFF0C\u6539\u52A8\u524D\u8BF7\u5148\u624B\u5DE5\u4FEE\u590D";
        }
        const found = findEntries(doc).map((e) => entryToView(e, text));
        for (const view of found) view.toolCount = countTools(view.serverName);
        servers.push(...found);
      } catch (e) {
        fileIssue = "patch \u6587\u4EF6\u89E3\u6790\u5931\u8D25\uFF08" + String(e?.message ?? e).slice(0, 200) + "\uFF09";
      }
    }
  ...
  async function persistAndSnapshot(doc, beforeCount, path) {
  ...
  try {
    ...
    try {
      console.error("[dshp-mcp-manager] register routes failed: " + String(e?.message ?? e));
    } catch {
    }
  }
}
