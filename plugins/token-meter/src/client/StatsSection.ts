...
export function createStatsSection(React: AnyReact, P: AnyPrimitives, ReactDOM: any): Record<string, any> {
  ...
  function AnimatedNumber(props: {
    ...
    return h(
      'div',
      { className: (props as any).className || '', style: (props as any).style || undefined },
      format(disp),
    );
  }
  ...
  function Heatmap(props: {
    ...
    for (let r = 0; r < 7; r++) {
      ...
      for (let w = 0; w < weeks; w++) {
        const cell = (
          cols[w] as Array<{ k: string; v: number; day: DayAgg | undefined; sess: number } | null>
        )[r] as { k: string; v: number; day: DayAgg | undefined; sess: number } | null;
        cells.push(
          h('div', {
            key: w,
            className: 'tm-hcell',
            'data-lv':
              cell === null || (cell as { v: number }).v <= 0
                ? '0'
                : String(levelOf((cell as { v: number }).v)),
            style:
              cell === null
                ? { visibility: 'hidden' }
  ...
  function HeatSection(props: {
    ...
    return h(
      ...
      h(
        ...
        HEAT_OPS.map((o, i) =>
          h('span', { key: i, className: 'tm-cell', style: { background: BP, opacity: o } }),
        ),
      ...
      h(
        ...
        h(
          Btn,
          {
            ...
            disabled: loading,
            icon:
              loading && P.IconLoadingOutline16
                ? h(P.IconLoadingOutline16)
                : P.IconRefreshOutline14
                  ? h(P.IconRefreshOutline14)
                  : undefined,
          },
          loading ? '刷新中…' : '刷新',
        ),
          ...
          ? h(
              ...
              (data as StatsSnapshot).errors > 0
                ? h(
                    'span',
                    { className: 'tm-muted' },
                    (data as StatsSnapshot).errors + ' 个会话读取失败已跳过',
                  )
                : null,
            )
          : null,
              ...
              return tmPortal(
                h(
                  ...
                  h(
                    ...
                    h('span', { className: 'tm-tip-v' }, fmt(todayTotal)),
                  ),
                  h(
                    'div',
                    { className: 'tm-tiprow' },
                    h('span', { className: 'tm-tip-k' }, '昨日'),
                    h('span', { className: 'tm-tip-v' }, fmt(yTotal)),
                  ),
                  h(
                    'div',
                    { className: 'tm-tiprow' },
