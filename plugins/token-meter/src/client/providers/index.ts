 ...
 *   Client 侧 providers/ui/<type>.tsx     **该供应商专属的完整 UI 实现**（自由排版）
 *            providers/kit.tsx            通用可复用零件（滚动刷新条/余额块/指标/图表/失败卡…）
 *            providers/sections.tsx       声明式区块渲染（无专属 UI 的供应商零代码可用）
 *            providers/registry.tsx       调度：专属 UI → 声明式 → legacy 兜底
 *
 * 扩展方式：
 *  - 只想「能显示」：Host 适配器产出 view 即可，客户端零改动（走声明式层）；
 *  - 想要「专属界面」：在 ui/ 下加一个 `<type>.tsx` 并在 ui/index.ts 注册一行，
