...
export function createTokenMeterSection(React: AnyReact, P: AnyPrimitives, ReactDOM: any): any {
  ...
  function TokenMeterSettings(): any {
    ...
    React.useEffect(() => { void store.ensureLoad(); }, []);
    ...
    kids.push(h('div', { key: 'display', className: 'tm-section' },
      ...
      h(UI.SecRow, { key: 'sec', label: '自动刷新', desc: '右栏打开时，当前供应商按间隔自动拉取；0=关闭。' },
        h(UI.PillSelect, {
      ...
      h(UI.SecRow, { key: 'range', label: '默认范围', desc: '用量面板打开时的默认时间范围。' },
        h(UI.PillSelect, {
          disabled: busy, value: defRange,
    ...
    const vrows: any[] = [
      h(UI.SecRow, {
        key: '__active', label: '当前供应商', desc: '侧边栏/右栏额度卡展示的额度来源；无 = 极简模式。',
      }, h(UI.PillSelect, {
        disabled: busy, value: activeId,
        selectedLabel: a0 ? a0.name + '（' + store.typeLabel(a0.type) + '）' : '无 · 极简模式（不拉取）',
        options: [{ id: '', label: '无 · 极简模式（不拉取）' }].concat(vendors.map((v: any) => ({ id: v.id, label: v.name + '（' + store.typeLabel(v.type) + '）' }))),
        onSelect: (id: string) => void withBusy(async () => { await store.setActive(id); await store.reload(); }),
      })),
    ];
    vendors.forEach((v: any) => {
      ...
      vrows.push(h(UI.SecRow, {
        ...
        h('span', { className: 'tm-bar', style: { width: '100%', padding: 0, border: 'none' } },
          ...
          h(Btn, { variant: 'ghost', size: 'sm', disabled: busy, onClick: () => { setAcked(false); setConfirmDel(v); } }, '删除'))));
    });
    ...
    if (confirmDel) {
      ...
      if (P.RiskConfirmation) {
        kids.push(h(P.RiskConfirmation, {
          key: 'confirm-del', open: true,
          title: '删除供应商「' + target.name + '」？',
          description: '该供应商的配置与侧边栏快照将一并清除，此操作不可撤销。',
          acknowledgeLabel: '我已了解，删除该供应商',
          cancelLabel: '取消', closeLabel: '关闭', confirmLabel: '删除',
          acknowledged: acked, onAcknowledgedChange: setAcked,
        ...
        kids.push(h('div', { key: 'confirm-del', className: 'tm-errbox' },
          ...
          h('div', { style: { display: 'flex', gap: 8, marginTop: 6 } },
            acked
              ? h(Btn, { variant: 'outline', size: 'sm', onClick: () => { setConfirmDel(null); setAcked(false); void delVendor(target.id); } }, '确认删除')
  ...
  function StatsRightPane(): any {
    const [prefs, setPrefs] = useState<{ showToday: boolean; defaultRange: string }>({ showToday: false, defaultRange: '30' });
