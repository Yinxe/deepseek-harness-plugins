...
**入口**：会话顶部的活动栏上一个图标（本插件唯一的图标，`token-meter:menu`）。点开是一片
**小组件菜单**：逐张列出本插件注册的卡片（开着的一行显示状态点 + 「收起」，其余显示「打开」），
底部一行「N / M 已打开」+「全部收起」。想开哪张就开哪张，不用先钻进「Token 总览」那个 tab；
中心区各卡片与图表标题栏上的「⧉」仍然可用（两个入口操作同一批卡片）。
...
| 峰谷定价（`peak`）         | 440 × 200 | 320 × 160  | 一行标题 + 24 小时色带 + 两行说明                        |
| 供应商额度（`quota:<id>`） | 400 × 320 | 320 × 220  | 标题行（名称 / 徽标 / 三个按钮）+ 若干窗口进度条         |
