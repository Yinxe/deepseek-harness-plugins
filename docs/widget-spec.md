...
| widget-kit 的 Host 半       | 只影响**框架偏好**（外观/动效/托盘上限存条目 `config:`）与 `/ext/dshp-widget-kit/*`；不影响注册与渲染 |
...
设置 → 小组件 → 「卡片外观与动效」里可调，全部写进 profile 条目 config，刷新与换设备都保持：
...
| 数据                                                                                                                     | 存哪                   | 键/位置                | 说明                                                                                    |
| ------------------------------------------------------------------------------------------------------------------------ | ---------------------- | ---------------------- | --------------------------------------------------------------------------------------- |
| 用户**配置**：托盘开关、图标上限、徽标间隔、悬停预览、参考组件、**卡片背景透明度 / 毛玻璃模糊 / 边框 / 圆角 / 动效时长** | profile 条目 `config:` | `dshp-widget-kit` 条目 | 走宿主 `/ext/dshp-widget-kit/state`、`POST config`；跨设备、随 settings 备份            |
...
- **0.12.0（框架版本，契约仍为 v1）**：无契约变更 —— 随 DSH 0.1.7（rc.1）适配升版（Host 配置改走条目 `Config` + volatile 契约），仅版本号与框架对齐。
...
- **0.9.0（框架版本，契约仍为 v1）**：新增五项**用户偏好**统一控制卡片/小面板外观与动效
  ...
  拖动滑块只改本地预览、松手才写条目 config。Host 与 Client 两侧的偏好默认值由
