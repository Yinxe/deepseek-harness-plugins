...
### 移动、吸附与防重叠（框架行为，提供方不用管）

拖动（以及键盘方向键微调）时，落点由框架的 `dockRect` 决定：**夹进视口 → 吸附 → 避让**，
三件事都在**拖动期间每帧**算，所以吸附效果是**提前渲染**出来的 —— 松手只是把同一份几何写进本机布局。

1. **吸附（磁力）**：同轴（横向的左右边、纵向的上下边）与**视口边缘**或**邻卡边缘**相差 ≤ `12px`
   就吸过去；贴邻卡时留 `8px` 间隔。若另一轴也相差 ≤ `28px`，顺带**对齐**（上对齐 / 下对齐 / 左对齐 / 右对齐）。
   例：把卡片拖到另一张卡右侧、两者顶边差 4px → 落到「右侧贴边 + 上对齐」。
2. **防重叠**：任何时刻都不允许压在别的卡片上。目标位置与邻卡冲突时，取「贴到某张邻卡某一侧」里
   位移最小的空位（这一轴贴边、另一轴在容差内对齐）—— 所以靠近时是**贴到旁边**，而不是叠上去。
3. **视口约束**：整卡不得离开视口；屏幕真的放不下时（无空位可贴）保持原样：宁可短暂重叠，也不乱跳。
4. **谁参与避让**：当前打开且**未最小化**的卡片（最小化的只留一条标题栏，不算障碍）。
5. **新建卡的落点**：先按层叠锚点算，再走同一套吸附 / 避让 —— 新卡宁可贴到旁边，也不会压在已有卡片上。
6. **键盘微调**（方向键 / Shift+方向键）只受「视口 + 防重叠」约束，**关掉磁力**，否则贴着邻卡时会每步被吸回去；
   `⋯` 菜单的「居中」同理（不被磁力拉走，但仍然不压别人）。
7. **缩放不受吸附影响**：缩放只夹在 `min/max` 与视口内（对边固定是缩放的硬语义）。
8. 视口变化（`resize`）只做「夹回视口内」，不会重新吸附 —— 布局是用户摆的，不该被窗口大小改掉。
...
7. **吸附只做「贴边 + 对齐 + 防重叠」**：不做自由网格、不做等宽等高的网格对齐、不做分屏/停靠 tab 化；
   屏幕塞满时允许短暂重叠（无解时以「不跳」优先）。
