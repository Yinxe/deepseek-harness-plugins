...
| 其它插件（参考组件…）       | 与本契约无关。谁都能用 `ctx.widgets.open/close/toggle(id)` 打开你注册的卡片，别人不参与注册             |
...
| `listedInBox`               | `boolean`                                              | —                               | `false`                | 允许被框架的统一入口「组件箱」收录（只有 `card` 能写）；**默认不收**，见 §6 |
...
4. 最小化 = **收成一枚胶囊**：高度 36px、两端全圆，宽度**量出来的** ——
   「整条标题栏需要多宽」（内边距 + 标题 + 间隙 + 真实按钮），夹在 `160px`（地板：再短的标题也给得出手）与
   **布局里的展开宽度**（上限：胶囊不比你自己选的卡片宽）之间。所以长标题的卡片不会收成一条把标题截成
   省略号的窄条，锁定时按钮变少（只剩「解锁 + ⋯」）胶囊也跟着窄一点。
   **宽度必须写成这个数字，而不是 `width: auto`** —— 浏览器在「长度 ↔ auto」之间无法插值，
   用 auto 时宽度会先瞬移、再慢慢缩高度，观感很跳；量出来的长度让宽高同一条曲线一起收，才顺。
   框架怎么知道要量多宽：卡片渲染时会克隆一份标题栏、放到屏幕外按 `width: max-content` 排一次版，
   量到的就是「标题不被省略号截掉」所需的宽度；**不按任何手写常量做加法**（常量与真实排版差一两像素，
   在浏览器里就表现为「标题被截」），量不到就退回地板宽度。量到的值再加 6px 余量吸收整数取整 /
   卡片边框 / 网络字体换装带来的 1px 级误差；万一仍然被省略号截着（字体晚到、外观偏好改了边框），
   框架会读标题**自己的溢出量**补一次差 —— 只在胶囊还没顶到布局宽度时补，所以单调、有界、不会振荡。
   这条对提供方是透明的 ——
   你不需要回报任何尺寸（也**不要**在 `render` 里 `setSize`，那是禁止的回环）。
...
- **最小化**：收成一枚**胶囊**（高度 36px、两端全圆、宽度**量出来的**：夹在 160px 地板与
  卡片展开宽度之间，标题多长胶囊就多宽，不会被固定宽度挤成省略号），
...
4. **让框架的统一入口「组件箱」替你挂出来**：`trayIcon: false` + `listedInBox: true` —— 你没有自己的菜单，
   ...
   **`listedInBox` 默认 `false`，而且必须自己声明。** 入口归卡片的所有者决定：框架不会把别人的卡片
   当成自己菜单的数据源，也不会在你没声明的时候替你把卡片挂到某个公共入口上。声明是**一步到位**的契约：
   `ctx.widgets.list()` 的摘要里带这个标记（`WidgetSummary.listedInBox`），所以别的聚合入口
   （比如你自建的菜单）也能按同一条声明决定收不收，不用读框架内部。

   「组件箱」是**框架能力，不是参考组件**：它不受「装载参考组件」偏好影响，始终在册 ——
   否则关掉参考组件会让声明过收录的卡片彻底没有入口。`minFramework: '0.11.0'` 是这条声明的版本门槛
   （更早的框架不认识这个字段，会**静默忽略**，于是卡片没有任何入口）。
...
9. **「组件箱」只列声明过 `listedInBox: true` 的卡片**（默认 `false`）：框架不会把第三方卡片收进
   自己的入口 —— 要自建菜单就只写 `trayIcon: false` 再注册一个 popover，要让框架替你挂就声明收录。
   声明只表示「允许收录」，收不收由入口决定；组件箱不看 `trayIcon`（有自己图标的卡片声明了也会被列出）。
...
- **0.10.2（框架版本，契约仍为 v1）**：修「折叠后标题被挤压 / 省略」。0.9.1 为了动画把胶囊宽度定成
  `min(布局宽度, 160px)` 的固定值，标题栏那几个 24px 按钮一占，标题只剩几十像素 —— 长标题必被截成省略号。
  现在胶囊宽度是**量出来的**：卡片渲染时克隆一份标题栏、放到屏幕外按 `width: max-content` 排一次版，
  量到的就是「标题不被省略号截掉」所需的宽度（含真实按钮、内边距、间隙与卡片自己的 0.5px 边框），
  再夹在 `160px` 地板与布局展开宽度之间。宽度仍是显式数字，所以最小化动画与 0.9.1 一样顺。
  **为什么不用「量标题文字 + 按常量加内边距 / 按钮」**：那条路会偏窄 2–18px，量出来的宽度照样把标题截掉 ——
  两个真原因都在浏览器里实测过：① `Range` 量到的是**墨迹宽度**，而 flex 用的 `max-content` 是 **advance 宽度**
  （差 1–2px）；② 官方 primitives 的按钮一旦不是 24px、`Menu` 锚点外多一层包装，手写常量就整段失效。
  量整条标题栏天然免疫这两件事，也不再需要「CSS ↔ 常量」的静态对齐门禁（只剩标题栏高度那一项还盯）。
  量到的值另加 **6px 余量**（整数取整 / 卡片 0.5–1px 边框 / 网络字体换装都会让精确值差一两像素，
  而差 1px 就足以让 `text-overflow: ellipsis` 生效）；并在 `fonts.ready` 的**下一帧**与 `loadingdone`
  时重量一次（`ready` 解决的那一刻布局可能还是回退字体，同步量会量到旧宽度）。最后还有一层兜底：
  最小化时若标题**仍然**被省略号截着，读标题自己的 `scrollWidth − clientWidth` 补一次差 ——
  只在胶囊还没顶到布局宽度时补，补完重量一次、溢出归零即停，因此单调有界、不会振荡。
  量不到（没有 DOM 的宿主、元素已摘掉）就不回报，胶囊退回地板宽度。另修一处会让测量**永久失效**的 bug：
  卸载时删掉测量、而同 id 重注册复用同一个 `Card` 实例（effect 依赖里只写了 `widget.id`）不会重量 ——
  token-meter 在供应商增删时会重注册，这类卡片的胶囊就永远停在地板宽度上。现在 effect 依赖带 `widget` 对象本身。
  内部改动，提供方**不需要**做任何事（`props.minimized` 语义不变）。
